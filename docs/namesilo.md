...
- `"cleanup"` (boolean, defaults to `false`) makes the updater proactively prune **stale** A/AAAA records for the host. By default the updater considers a host "up to date" as soon as your public IP appears among its records, so any extra records lingering at the same host (e.g. a duplicate left by an older client, or a manual edit) are never noticed and never removed. With `"cleanup": true`, whenever DNS resolves _more than one_ record for the host the updater forces a reconcile that deletes the stale records and keeps a single one pointing at your current IP. Only enable this for hosts that must resolve to exactly one record — do **not** enable it for hosts that intentionally use multiple records (e.g. round-robin). Note: because public resolvers cache answers for the record's TTL, a freshly-pruned host may keep triggering a (cheap, read-only) reconcile each cycle until the cached duplicate expires; raising `UPDATE_COOLDOWN_PERIOD` toward your TTL minimizes this.
